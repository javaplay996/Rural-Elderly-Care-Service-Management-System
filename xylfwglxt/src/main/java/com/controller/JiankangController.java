    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(jiankang !=null){
            ...
            //级联表 医疗人员
            //级联表
            YiliaoEntity yiliao = yiliaoService.selectById(jiankang.getYiliaoId());
            if(yiliao != null){
...
, "yiliaoId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 老人
            //级联表
            LaorenEntity laoren = laorenService.selectById(jiankang.getLaorenId());
            if(laoren != null){
    ...
    */
    ...
    public R save(@RequestBody JiankangEntity jiankang, HttpServletRequest request){
        ...
        if(false)
            ...
            jiankang.setLaorenId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("医疗人员".equals(role))
            jiankang.setYiliaoId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<JiankangEntity> queryWrapper = new EntityWrapper<JiankangEntity>()
            .eq("yiliao_id", jiankang.getYiliaoId())
            .eq("laoren_id", jiankang.getLaorenId())
            .eq("jiankang_name", jiankang.getJiankangName())
            .eq("jiankang_address", jiankang.getJiankangAddress())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            JiankangEntity jiankangEntity = new JiankangEntity();
//                            jiankangEntity.setYiliaoId(Integer.valueOf(data.get(0)));   //医疗人员 要改的
//                            jiankangEntity.setLaorenId(Integer.valueOf(data.get(0)));   //老人 要改的
//                            jiankangEntity.setJiankangName(data.get(0));                    //健康档案名称 要改的
//                            jiankangEntity.setJiankangUuidNumber(data.get(0));                    //健康档案编号 要改的
//                            jiankangEntity.setJiankangPhoto("");//详情和图片
//                            jiankangEntity.setJiankangAddress(data.get(0));                    //健康档案地点 要改的
//                            jiankangEntity.setJiankangTypes(Integer.valueOf(data.get(0)));   //身体情况 要改的
...
//                            jiankangEntity.setCreateTime(date);//时间
                        ...
                        if(jiankangEntities_jiankangUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [健康档案编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
